...
album_feature_chart <- function(df, feature) {
  ...
  plot_df <- df %>% 
    ...
    rowwise %>% 
    mutate(tooltip = paste0('<a style = "margin-right:', max(nchar(as.character(track_name)), nchar(as.character(album_name))) * 9, 'px">',
                            '<b>Album:</b> ', album_name,
                            '<br><b>Track:</b> ', track_name)) %>% 
    ungroup
  avg_line <- plot_df %>% 
    group_by(album_rank, album_name) %>% 
...
artist_quadrant_chart <- function(track_df) {
  ...
  track_df %>% 
    rowwise %>%
    mutate(tooltip = paste0('<a style = \"margin-right:', max(max(nchar(as.character(track_name)), nchar(as.character(album_name))) * 4, 110), 'px\">',
...
classify_track_sentiment <- function(valence, energy) {
  if (is.na(valence) | is.na(energy)) {
    ...
    if (energy >= .5) {
      return('Alegria/Excita??o')
    } else {
      return('Tranquilidade')
    }
  } else {
    if (energy >= .5) {
      return('Agita??o/Raiva')
    } else {
      return('Tristeza')
    }
  }
