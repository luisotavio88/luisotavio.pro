...
Existem outras estruturas de controle que não serão abordadas aqui. Porém essas são as mais importante.
...
Porém, em alguns casos, vai ser mais interessante manter o looping rodadando **enquanto** alguma condição ainda não foi atendida. **Para isso usaremos a função ```while```.**
...
Ou seja, você irá estabelecer uma condição e, caso ela seja atentida, o código será executado.
...
Agora, vamos adicionar a função ```else``` que é simplemente o SENÃO. Caso a condição lógica seja FALSA, o *script* contido dentro da função ```else``` é executado.
...
Lembre também que caso você esteja trabalhando com um data frame, cada coluna de seu data frame pode ser tratada como um vetor.
